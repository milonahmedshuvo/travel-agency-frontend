...
const TourStripeEightyPaymentPage = () => {
  ...
  const amount = useAppSelector(
    (state) => state.booking.tourBookingEightyPayment?.amount
  );
  const clientSecret = useAppSelector(
    (state) => state.booking.tourBookingEightyPayment?.clientSecret
  );
  ...
  console.log(
    "filnal transition id",
    tourBooking?.data?.splitPayment?.finalPaymentTransaction?.id
  );
      ...
      if (result.paymentIntent?.status === "succeeded") {
        ...
        try {
          const res = await fetch(
            `${getBaseUrl()}/tour-bookings/${id}/split-pay/final`,
            {
              method: "POST",
              headers: {
