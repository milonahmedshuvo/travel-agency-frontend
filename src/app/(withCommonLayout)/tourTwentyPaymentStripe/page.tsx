...
const TourStripeTwentyPaymentPage = () => {
  ...
  const amount = useAppSelector(
    (state) => state.booking.tourBookingPayment?.amount
  );
  const clientSecret = useAppSelector(
    (state) => state.booking.tourBookingPayment?.clientSecret
  );
      ...
      if (result.paymentIntent?.status === "succeeded") {
        ...
        try {
          const res = await fetch(
            `${getBaseUrl()}/tour-bookings/${id}/split-pay/initial`,
            {
              method: "POST",
              headers: {
              ...
              body: JSON.stringify({
                paymentMethodId,
                transactionId:
                  tourBooking?.data?.splitPayment?.initialPaymentTransaction
                    ?.id,
              }),
            }
          );
