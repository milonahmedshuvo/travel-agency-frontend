...
import {
  setRoomBookingEightyPayment,
  setRoomBookingPayment,
...
export default function AccommodationPaymentCard() {
  ...
  const handleRoomBookingByStripe = () => {
    ...
    const makeStripePayment = async () => {
      ...
      const url = `${getBaseUrl()}/room-bookings/full-payment/${id}/stripe`;
  ...
  const handleRoomBookingByPaypal = () => {
    ...
    const makeStripePayment = async () => {
      const token = localStorage.getItem("token");
      const url = `${getBaseUrl()}/room-bookings/full-payment/${id}/paypal`;
  ...
  const handlePaymentSelection = (methods: {
    ...
    if (methods.initialPaymentMethod === "STRIPE") {
      const makeStripePayment = async () => {
        const token = localStorage.getItem("token");
        const url = `${getBaseUrl()}/room-bookings/${id}/split-pay`;
